...
class Map //Estrutura de armazenamento das chaves do mapa, baseada em uma RwayTrie. Esse mapa suporta apenas chaves com dois caracteres, no qual o primeiro é uma letra minúscula de 'a' a 'j', e o segundo é um número de 0 a 9, ou seja, foi feito específicamente para uma tabela 10 x 10.
{
  ...
  {
			let t = nav.tam;
			let line = getRandom(97,106); //aleatoriza uma linha
			line = String.fromCharCode(line);
			let q = this.keysThatMatch(line + '.'); // armazena as chaves existentes na linha
			let list = new LSE(); // utilizada para armazenar as chaves que não possuem valor associado
			for(let i = 0; i < 10; i++){
				let box = line + i;
				if(!q.contElem(box)) list.add(i);
			}
			let seq = list.longestSeq();
			if(t <= seq.tam) //checa se a sequência de casas livres comporta o navio
      {
				let n = seq.finish + 1 - t;
				let init = seq.finish - seq.tam + 1;
				let rand = getRandom(init, n); //posição aleatoria para inicio da distribuição das peças do navio
  ...
  {
			let t = nav.tam;
			let col = getRandom(0,9); //aleatoriza uma coluna
			col = col.toString();
			let q = this.keysThatMatch('.' + col); // armazena as chaves existentes na coluna
			let list = new LSE(); // utilizada para armazenar as chaves que não possuem valor associado
